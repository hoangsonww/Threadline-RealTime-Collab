...
- **`AUTH_DELIVERY_WEBHOOK` isn't configured on the live deployment, so no mail is sent at all.** Password-reset requests correctly return `202` (by design — never leaking account existence, see [`security.md`](security.md#password--credential-storage)), but nothing is delivered and the token expires unused, which means **account recovery does not work in production**. Wiring up a real transactional-email provider is a separate integration decision this deployment hasn't made. Email verification had the same silent failure and was removed rather than left reporting success; recovery can't be removed the same way. See [`roadmap.md`](roadmap.md).
