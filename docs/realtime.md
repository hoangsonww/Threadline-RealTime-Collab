...
- **No progress reporting during a file transfer.** The outcome _is_ reported — `uploadFiles()` moves a sent file to "Sent to N connected peers", or "No peer was ready — try again" when nobody received it. What is missing is anything in between: no percentage, no per-peer breakdown, and so no way to tell a slow transfer from a stalled one while it is running.
