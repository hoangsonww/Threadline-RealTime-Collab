...
- **Direct P2P needs a TURN server in real-world network conditions, and that isn't wired up yet.** `PeerMesh`'s constructor accepts an optional `iceServers` list, but `room-workspace.tsx` never passes one — every session today uses the single hardcoded default, a public Google STUN server (`stun:stun.l.google.com:19302`). STUN alone is enough for most home/office networks but not for symmetric NATs or locked-down corporate networks, which need a TURN relay to connect at all. Shipping TURN support means both threading a configured `iceServers` list from `apps/web` into `new PeerMesh({...})` and operating (or paying for) a TURN service — neither exists today.
...
- **Room membership has no revoke UI.** `POST /v1/rooms/:roomId/members` grants access; there's no corresponding `DELETE`. Removing someone from a restricted room today means removing them from the organization.
