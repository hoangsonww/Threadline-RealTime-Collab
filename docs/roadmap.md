...
| Operability   | Email delivery isn't configured on the live deployment     | `AUTH_DELIVERY_WEBHOOK` is unset there, so no email sends at all; that requires a real transactional-email provider account, a separate integration decision. **Password recovery therefore does not work in production** — the request returns `202` by design (to avoid leaking account existence) and the token expires undelivered. Email verification previously failed the same way and has been removed rather than left lying; recovery cannot be removed the same way, so it is the top gap here.                                                                                                                                                                                       | [`operations.md`](operations.md#known-gaps-carried-here-from-live-testing) |
