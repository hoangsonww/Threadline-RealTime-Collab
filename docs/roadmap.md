...
quadrantChart
    ...
    Room membership revoke: [0.3, 0.75]
...
| ------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| Room access   | No way to _revoke_ explicit room membership                | `POST /v1/rooms/:roomId/members` exists; there's no matching `DELETE`. Today, removing someone from a restricted room means removing them from the organization entirely.                                                                                                                             | [`api.md`](api.md#organizations--rooms), [`realtime.md`](realtime.md#known-limitations) |
| WebRTC        | TURN isn't wired up                                        | `PeerMesh` accepts an `iceServers` override, but nothing in `apps/web` currently populates one from configuration — every session uses the hardcoded public STUN default. Works on most networks; fails on symmetric NATs / locked-down corporate networks.                                           | [`realtime.md`](realtime.md#known-limitations)                                          |
...
| File transfer | No delivery confirmation                                   | `uploadFiles()` marks a sent file "Sending" once and never updates that status, even after the whole chunked transfer completes. Can't currently tell "sent," "still sending," and "peer never got it" apart in the UI.                                                                               | [`realtime.md`](realtime.md#screen-sharing)                                             |
...
| Operability   | Email delivery isn't configured on the live deployment     | `AUTH_DELIVERY_WEBHOOK` is unset there — password-reset and email-verification requests return a normal `202` (by design, to avoid leaking account existence), but no email actually sends, because that requires a real transactional-email provider account, a separate integration decision.       | [`operations.md`](operations.md#known-gaps-carried-here-from-live-testing)              |
